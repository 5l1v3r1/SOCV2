...
class PassGen:
    ...
    def generator(self, ignore_additional = True):
        ...
        if not ignore_additional:
            ...
            bar = progressbar.ProgressBar(maxval=max_count_default,widgets=[Fore.RESET+'Writing:',progressbar.Bar(left=Fore.RED+'[', marker=Fore.GREEN+'#', right=Fore.RED+']'),]).start()
            ...
            if self.silent:
               ...
               bar.finish()
        print(Fore.GREEN+'\nPasswords were Generated in file: {}'.format(output_file)+Fore.RESET)
        social.check_back('info')
